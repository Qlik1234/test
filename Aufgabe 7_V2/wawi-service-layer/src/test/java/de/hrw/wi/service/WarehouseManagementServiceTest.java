...
public class WarehouseManagementServiceTest {
	...
	public void setup() {
		...
		when(dbReadMock.getArticleCodeForBin(123, 2)).thenReturn(
				"0799637096608");
		when(dbReadMock.getArticleCodeForBin(456, 1)).thenReturn(
				"0799637096608");
		when(dbReadMock.getArticleCodeForBin(123, 1)).thenReturn(
				"1234567895608");

		when(dbReadMock.getAllProducts()).thenReturn(productCodes);
		when(dbReadMock.getNameOfProduct("0799637096608"))
				.thenReturn("Nutella");
		when(dbReadMock.getNameOfProduct("1234567895608")).thenReturn(
				"Marmelade");
